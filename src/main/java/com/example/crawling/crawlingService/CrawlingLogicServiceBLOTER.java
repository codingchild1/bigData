...
@Service
public class CrawlingLogicServiceBLOTER {
	private static final Logger logger = LoggerFactory.getLogger(CrawlingLogicServiceBLOTER.class);
	...
	public void crawlWebsite() throws NullPointerException {
		...
		String bloterNews = "newsByBloterNews";
		CrawlingEntity newsNoByBloterNewsList = crawlingRepository.findByMedia(bloterNews);
		long firstNewsNoByBloterNews = newsNoByBloterNewsList.getNewsNo();  //크롤링 시작 번호
		long newsNoByBloterNews = newsNoByBloterNewsList.getNewsNo();       //크롤링 데이터 카운트
		long lastNewsNoByBloterNews = newsNoByBloterNews + 6;              //크롤링 끝 번호
...
//		crawledNewsDataRepository.deleteAll();    //데이터 삭제
		...
		for (long newsNo = (int) newsNoByBloterNews; newsNo <= lastNewsNoByBloterNews; newsNo++) {
			try {
				// 현재 날짜/시간
				...
				//크롤링을 하려는 주소
				final String url = "https://www.bloter.net/news/articleView.html?idxno=" + newsNo;
...
//                Element imageUrl = document.getElementsByAttributeValue("alt", "동영상 링크").first();
				...
				String detail = document.select("div.article-body > article#article-view-content-div").text();
				...
				//기자 이름
				String reporter = document.select("div.account > strong.name").text().split(" ")[0];
				String strRepoter = reporter;
//                    strRepoter = strRepoter.replaceAll("기자", "");

				//기자 이메일
				String email = document.select("div.account > a.email").text();

				//언론사
				String media = document.select("div.user-logo > a > img").attr("alt");

				//기사 이미지 URL
				String imgUrl = document.select("figure.photo-layout > div.IMGFLOATING > img").attr("src");
...
//                    bookRepository.save(book);
				...
				newsNoByBloterNews += 1;
				crawlingEntity.setMedia("newsByBloterNews");
				crawlingEntity.setNewsNo(newsNoByBloterNews);
...
//			for (Book books : bookRepository.findAll()) {
			for (long newsNo = firstNewsNoByBloterNews; newsNo < lastNewsNoByBloterNews+1; newsNo++) {
				try {
